...
**Correção seguinte (2026-08-29):** o usuário reconferiu — o valor certo é **R$ 707,00**. `UPDATE public.contas` na linha `chave='cofre_notas'` (id `4356ff96-…`): `saldo_inicial` 577 → **707,00**, `saldo` 427 → **557,00** (+130,00, para o saldo corrente acompanhar; 707 − 140 − 10). Mesmo mecanismo da correção anterior. Nenhum lançamento criado (sem receita/Pix/retirada), os 2 lançamentos do cofre (−140 "cesar impressora", −10 "pilha cortador") intactos, e Sicredi/PagSeguro/Cofre de Moedas/Fundo de Caixa/Caixa da Loja não tocados por esta operação. Estado das contas depois: Sicredi `saldo_inicial −994 / saldo −1.073,11` (o usuário re-registrou a gráfica como −79,11, lançamento novo id `70616936-…`) · PagSeguro `0 / 0` · **Cofre de Notas `707 / 557`** · Cofre de Moedas `8,50 / 8,50` · Fundo de Caixa `160 / 160` · Caixa da Loja `0 / 160`.
...
## Limpeza do fechamento errado de 29/08 — reset ao pré-fechamento (2026-08-29) — operação de dados, autorizada pelo usuário

Investigação: o "Caixa da Loja" aparecia com **R$ 151,15**. Diagnóstico — `public.contas.saldo` da linha `chave='caixa_loja'` (mantido pelo trigger `trg_lancamentos_saldo`/`fn_lancamento_ajusta_saldo`), exibido por `loadFinanceiroSaldos()` sem cálculo no cliente. Causa: o lançamento `RECEITA_SERVICOS` de caixa do fechamento de 29/08 (id `cc55f6cb…`, "Receita de serviços apurada no fechamento", origem `fechamento`, perna única) foi **editado pela tela Histórico Financeiro de 148,85 → 300,00**; o trigger jogou o delta `+151,15` no `caixa_loja.saldo` (estava 0). Como 29/08 é fechamento do **modelo antigo** (PASSO 4), `histReconciliarDia` tem trava para dias legados e não recalculou o `FECHAMENTO` para compensar. Não era ajuste, não era saldo inicial, não estava duplicado no Fundo de Caixa (que tem 0 lançamentos, saldo = inicial 160).

Correção escolhida pelo usuário: **apagar o fechamento e refazer do zero** pelo modelo novo, em vez de empilhar correções sobre um fechamento do modelo antigo. Após 2 rodadas de dry-run (transação com ROLLBACK) e confirmação final, **executadas 6 exclusões por `id`** em `public.lancamentos` (`DELETE`, sem tocar em config, `saldo_inicial`, nem contas):
- 4 registros `data='2026-08-29' AND origem='fechamento'`: `98c6857b…` FECHAMENTO caixa −308,85 · `cc55f6cb…` RECEITA_SERVICOS caixa +300,00 · `d92e71d1…` RECEITA_SERVICOS sicredi +263,89 · `4947d699…` RECEITA_SERVICOS pagseguro +63,12.
- 2 pernas da transferência PagSeguro→Sicredi R$ 63,12 (par `40419262…`): `547abe68…` sicredi +63,12 · `aaece649…` pagseguro −63,12. (Removida junto porque só existiu por causa da receita apurada no PagSeguro que também foi apagada; o usuário vai refazê-la depois do novo fechamento.)

Estado resultante (confirmado): `lancamentos` 10 → **4** (ABERTURA +160 + as 3 retiradas reais: cofre_notas −140, cofre_notas −10, sicredi −79). Saldos: **Caixa da Loja 160,00** · **Sicredi −1.073,00** (−994 inicial − 79 gráfica, sem receita apurada ainda) · **PagSeguro 0,00** · **Cofre de Notas 427,00** · **Cofre de Moedas 8,50** · **Fundo de Caixa 160,00**. `saldo_inicial` de todas inalterado. `FECHAMENTO` no dia 29 = 0, `RECEITA_SERVICOS`/`AJUSTE_CAIXA` em todo o banco = 0, `origem='fechamento'` em todo o banco = 0 → **dia 29/08 aberto de novo, no pré-fechamento**.

**Pendente (o usuário fará):** refazer o fechamento de 29/08 pelo modelo de conferência (conferindo gaveta/bancos/cofre) e, só depois, refazer a transferência de R$ 63,12.
...
## Fechamento de Caixa — saldo de banco informado ABAIXO do sistema (inclusive negativo) (2026-08-31) — NÃO publicado

Bug relatado: no **Fechamento de Caixa diário**, ao informar o saldo atual de Sicredi/PagSeguro **menor que o saldo do sistema** (ex.: banco em −713,19), o sistema calculava a diferença certa mas só gravava se ela fosse **positiva** (`if (b.receita > 0.005)` em `finFecharCaixa`). A diferença negativa era descartada: a conta **nunca era ajustada** para o valor informado, e o `Math.max(0, b.receita)` no `finResultadoProjetado` zerava a variação negativa.

**Correção — só `paineldecontrole/index.html`, sem migração, sem tocar no caminho positivo:**
- `finFecharCaixa` — novo ramo `else if (b.receita < -0.005)`: grava um lançamento `AJUSTE_CAIXA` na conta do banco com `valor = informado − esperado` (negativo). O trigger `fn_lancamento_ajusta_saldo` faz `saldo += valor` pra qualquer tipo, então a conta fica **exatamente** no valor informado (aceita saldo negativo). `AJUSTE_CAIXA` já é classificado como **neutro no resultado** (`finClassificarLancamento` → `grupo:'ajuste'`, `valorEconomico:0`) em todos os agregadores (diário, projetado, `relAgregar`, mensal) — a identidade de consistência (`somaValores − abertura − fechamento − ajuste − resultado`) continua fechando porque o valor entra em `somaValores` e sai em `N.ajuste`.
- `finApurarReceitas`/meta do FECHAMENTO — split novo: `receitas_banco` (delta > 0), `ajustes_banco` (delta < 0), `bancos_nao_apurados` (|delta| ≈ 0).
- `mfFechamentoConferir` — aviso **não bloqueante** por banco: "Saldo informado de X (− R$ …) está abaixo do saldo do sistema (R$ …). Ao fechar, o saldo da conta é ajustado … neutra no resultado (não é despesa). Saldo negativo é aceito." O bloqueio de `notasInsuficientes` (notas < fundo) foi mantido intacto — o usuário confirmou que o problema era só o saldo de banco. `mfFechSicrediAtual`/`mfFechPagseguroAtual` passaram a usar `finFmtSaldo` (mostra `− R$` em vez de `R$ -`).
- Mensagem de sucesso do fechamento — acrescenta "Saldo de X ajustado para − R$ … (diferença …, neutra no resultado)".
- **Histórico Financeiro** — editar um fechamento salvo agora também simetriza: mantém **UMA** linha apurada por conta (`RECEITA_SERVICOS` se delta > 0, `AJUSTE_CAIXA` se delta < 0, nenhuma se ≈ 0), recalculando a partir do saldo do sistema menos o efeito da linha atual. Antes, um informado menor gravava `valor` negativo numa linha `RECEITA_SERVICOS` (que o classificador conta como `Math.abs` → viraria receita positiva). Os campos "Saldo Sicredi/PagSeguro" do editor ganharam `data-negativo="1"` (novo opt-in em `limparValorCentsDigitado`: preserva UM `-` à frente; sem o atributo o comportamento é idêntico ao de antes — sinal apagado). `atualizarExtensoValorCents` mostra "(negativo)" para valores < 0.
- `AJUSTE_CAIXA` com `origem:'fechamento'` já era removido junto ao excluir o FECHAMENTO no Histórico (linha existente do `lote`), e `histReconciliarDia` já pula `AJUSTE_CAIXA` — nada a mais a fazer nesses pontos. No Histórico as linhas de ajuste ficam **anexadas ao card do FECHAMENTO** (`op.ajusteLegs`, igual às pernas de cofre) em vez de virar card solto.

**Testado (static server, sem senha da equipe):**
- Página carrega sem `SyntaxError` (só o 404 estático pré-existente); todos os `window.fin*` presentes.
